...
def net_evaluation(net,test_loader,dataset_size,test_batch_size = 500):
    ...
    #     x = torch.tensor([item.cpu().detach().numpy() for item in x]).cuda();
        ...
        x = x.cuda()
        h = net.resnet(x)
        # 分层train_layerResnet专用语句
        # h1,h2 = net.resnet(x);
        #h2 = nn.AdaptiveAvgPool2d((1, 1))(h2)
        #h2 = torch.flatten(h2, 1)

        # print("h1,h2",h1.shape,h2.shape)  # [500,256],[500,512]
        c = net.cluster_projector(h);
        # c = net.cluster_projector(h2);
        c = torch.argmax(c, dim=1)
    ...
    # path = "./Dogs-1000epoch.txt";
    # with open(path,'a') as f:
    #     f.write(str(my_acc)+' '+str(my_nmi)+' '+str(my_ari)+'\n');